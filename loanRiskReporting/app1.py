...
class CreditRiskTools:
    def __init__(self, config):
        ...
        # This method call works because the method is defined below it
    ...
    # The method is clearly defined within the class scope
...
def get_html_download_link(styled_df, current_base_score_value):
    html_string = styled_df.to_html(escape=False)
    b64 = base64.b64encode(html_string.encode()).decode()
    filename = f"loan_risk_report_BaseScore{current_base_score_value}.html"
...
st.markdown(f"**Current Configuration:** Base Score = `{st.session_state['config']['base_credit_score_value']}`, Economic Outlook Index = `{st.session_state['config']['economic_outlook_index']}`")
...
if uploaded_file:
    try:
        ...
        with col_html:
            st.markdown(get_html_download_link(styled_df, st.session_state['config']['base_credit_score_value']), unsafe_allow_html=True)
